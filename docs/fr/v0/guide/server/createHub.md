...
Le `Hub` est le coeur de votre application. Il centralise la configuration, les routes et les plugins. ces éléments ne dépendent pas de la plateforme d'exécution. Ils peuvent vivre dans n'importe quel environnement JavaScript. (Certaines fonctionnalités font exception et nécessitent quand même d'être sur un environnement serveur car elles créent des fichiers.).
...
::: info 
- Le plugin `codeGenerator` permet de créer un fichier de typage des entrée-sortie de vos routes.
